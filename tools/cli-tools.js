...
const handleConflicts = (filename) => {
	...
	console.log('\x1b[36m%s\x1b[0m', 'Comments with words like `Additions` reference text locations and need be immutable.')
...
exports.deportReplacers = (name) => {
	...
	var impIndex = data.indexOf('// importAdditions\r');
	var expIndex = data.indexOf('  // exportAdditions\r');
...
exports.createRoute = (name) => {
	...
	data.splice(rtIndex+1, 0, `    { path: '/`+name+`', name: '`+capitalize(name)+`', component: `+capitalize(name)+` },`);
	var text = data.join("\n");
